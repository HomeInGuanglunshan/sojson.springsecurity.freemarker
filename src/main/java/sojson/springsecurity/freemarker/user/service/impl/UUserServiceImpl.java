...
import sojson.springsecurity.freemarker.permission.bo.UserRoleAllocationBo;
...
public class UUserServiceImpl extends BaseMybatisDao<UUserMapper> implements UUserService {
	...
	public Map<String, Object> updateForbidUserById(Long id, Long status) {
		...
		try {
			...
			//如果当前用户在线，需要标记并且踢出
			sessionUtils.forbidUserById(id, status);
...
//		TokenManager.clearUserAuthByUserId(userId); // 登出后权限缓存自动清除
		resultMap.put("count", count);
		return resultMap;
	}
